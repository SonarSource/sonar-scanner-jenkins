...
If you only need the SonarQube environment variables to be expanded in the build context then you can override the `cli` flag.
```
withSonarQubeEnv('My SonarQube Server', cli: false) {
